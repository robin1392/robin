...
namespace JSONOBJECT
{
	...
	public class JSONObject : Nullable
	{
		...
		public static bool IsNull(JSONObject jsonObject) // for only list
		{
			return !(jsonObject != null && jsonObject.list != null && jsonObject.list.Count > 0 && jsonObject[0].type != JSONObject.Type.NULL);
		}
		...
		public JSONObject() { }
		public JSONObject(bool bn, string str)
		{
			type = Type.STRING;
			this.str = str;
		}
		...
		{   //create a new JSONObject from a string (this will also create any children, and parse the whole string)
			if(str != null)
			{
				...
				str = str.Replace("\\", "");
				int iLego = 65279;
				char chLego = (char)iLego;
				str = str.Replace(chLego.ToString(), "");
#endif
				if(str.Length > 0)
				{
		...
		public void AddField(double val) { Add(new JSONObject(val)); }
		public void AddField(int val) { Add(new JSONObject(val)); }
		//public void AddField(bool bn, string val) { Add(new JSONObject(bn,val)); }
		...
		public void AddField(string name, double val) { AddField(name, new JSONObject(val)); }
		...
		public void AddField(bool bn, string val) { Add(new JSONObject { type = JSONObject.Type.STRING, str = val }); }

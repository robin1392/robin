...
namespace RWCoreNetwork
{
    ...
    public class MessageHandler
    {
        ...
        public void OnReceive(UserToken userToken, byte[] buffer, int offset, int transfered, CompletedMessageDelegate callback)
        {
            ...
            while (_remainBytes > 0)
            {
                ...
                if (_currentPosition < Defines.PROTOCOL_ID + Defines.HEADERSIZE)
                {
                    ...
                    // 메세지 사이즈를 구한다.
                    _messageSize = BitConverter.ToInt16(_messageBuffer, Defines.PROTOCOL_ID);


                    // 다음 목표 지점
                    _positionToRead = _messageSize + Defines.PROTOCOL_ID + Defines.HEADERSIZE;
                }
                ...
                if (completed == true)
                {
                    callback(userToken, _messageBuffer);
                    ClearBuffer();
                }
            }
        }
        ...
        {
            if (_currentPosition >= _messageBuffer.Length)
            {

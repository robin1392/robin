...
namespace Service.Net
{
    ...
    public class ClientSession
    {
        ...
        public void SendNetAuthSessionReq(string gameSessionId)
        {
            ...
            using (var ms = new MemoryStream())
            {
                ...
                Send((int)ENetProtocol.AUTH_SESSION_REQ,
                    ms.ToArray(),
                    ms.ToArray().Length);
            }
        }

        public void SendNetAuthSessionAck(string gameSessionId)
        {
            var bf = new BinaryFormatter();
            using (var ms = new MemoryStream())
            {
                ...
                Send((int)ENetProtocol.AUTH_SESSION_ACK,
                    ms.ToArray(),
                    ms.ToArray().Length);
            }
        }
        ...
        public void SendNetDisconnectNotify()
        {
            var bf = new BinaryFormatter();
            using (var ms = new MemoryStream())
            {
                bf.Serialize(ms, (short)SessionState);
                Send((int)ENetProtocol.DISCONNECT_SESSION_NOTIFY,
                    ms.ToArray(),
                    ms.ToArray().Length);
            }
        }
        ...
        public void SendNetPauseSessionReq()
        {
            var bf = new BinaryFormatter();
            using (var ms = new MemoryStream())
            {
                bf.Serialize(ms, PauseStartTimeTick);
                Send((int)ENetProtocol.PAUSE_SESSION_REQ,
                    ms.ToArray(),
                    ms.ToArray().Length);
            }
        }
        ...
        protected void SendNetPauseSessionAck()
        {
            var bf = new BinaryFormatter();
            using (var ms = new MemoryStream())
            {
                Send((int)ENetProtocol.PAUSE_SESSION_ACK,
                    ms.ToArray(),
                    ms.ToArray().Length);
            }
        }
        ...
        public void SendNetResumeSessionReq()
        {
            var bf = new BinaryFormatter();
            using (var ms = new MemoryStream())
            {
                Send((int)ENetProtocol.RESUME_SESSION_REQ,
                    ms.ToArray(),
                    ms.ToArray().Length);
            }
        }
        ...
        public void SendNetResumeSessionAck()
        {
            var bf = new BinaryFormatter();
            using (var ms = new MemoryStream())
            {
                Send((int)ENetProtocol.RESUME_SESSION_ACK,
                    ms.ToArray(),
                    ms.ToArray().Length);
            }
        }

    ...
    public bool Send(ERandomWarsAccountProtocol protocolId, params object[] param)
    {
        switch (protocolId)
        {
            case ERandomWarsAccountProtocol.LOGIN_ACCOUNT_REQ:
                {
                    _randomWarsAccountTemplate.SendLoginAccountReq(NetService.Get().HttpClient, param[0].ToString(), (EPlatformType)param[1]);
                }
                break;
        }
        return true;
    ...
    public bool Send(ERandomWarsPlayerProtocol protocolId, params object[] param)
    {
        switch (protocolId)
        {
            case ERandomWarsPlayerProtocol.EDIT_NAME_REQ:
                {
                    _randomWarsPlayerTemplate.SendEditNameReq(NetService.Get().HttpClient, param[0].ToString(), param[1].ToString());
                }
                break;
        }
        return true;
    ...
    public bool Send(ERandomWarsDiceProtocol protocolId, params object[] param)
    {
        switch (protocolId)
        {
            case ERandomWarsDiceProtocol.UPDATE_DECK_REQ:
                {
                    _randomWarsDiceTemplate.SendUpdateDeckReq(NetService.Get().HttpClient, param[0].ToString(), (int)param[1], (int[])param[2]);
                }
                break;
            case ERandomWarsDiceProtocol.LEVELUP_DICE_REQ:
                {
                    _randomWarsDiceTemplate.SendLevelupDiceReq(NetService.Get().HttpClient, param[0].ToString(), (int)param[1]);
                }
                break;
            case ERandomWarsDiceProtocol.OPEN_BOX_REQ:
                {
                    _randomWarsDiceTemplate.SendOpenBoxReq(NetService.Get().HttpClient, param[0].ToString(), (int)param[1]);
                }
                break;
        }
        return true;
            ...
            case ERandomWarsMatchProtocol.REQUEST_MATCH_REQ:
                {
                    if (NetService.Get().NetMatchStep == Global.E_MATCHSTEP.MATCH_START
                        || NetService.Get().NetMatchStep == Global.E_MATCHSTEP.MATCH_CONNECT)
                    {
                        // 매칭 중이면 요청할 수 없다.
                        break;
                    }

                    NetService.Get().NetMatchStep = Global.E_MATCHSTEP.MATCH_START;
                    _randomWarsMatchTemplate.SendRequestMatchReq(NetService.Get().HttpClient, param[0].ToString());
                }
                break;
            case ERandomWarsMatchProtocol.STATUS_MATCH_REQ:
                {
                    if (NetService.Get().NetMatchStep == Global.E_MATCHSTEP.MATCH_CONNECT)
                    {
                        // 매칭 중에서 상태 요청을 할 수 있다ㅏ.
                        break;
                    }

                    _randomWarsMatchTemplate.SendStatusMatchReq(NetService.Get().HttpClient, param[0].ToString());
                }
                break;
            case ERandomWarsMatchProtocol.CANCEL_MATCH_REQ:
                {
                    if (NetService.Get().NetMatchStep == Global.E_MATCHSTEP.MATCH_CANCEL)
                    {
                        break;
                    }

                    NetService.Get().NetMatchStep = Global.E_MATCHSTEP.MATCH_CANCEL;
                    _randomWarsMatchTemplate.SendCancelMatchReq(NetService.Get().HttpClient, param[0].ToString());
                }
                break;

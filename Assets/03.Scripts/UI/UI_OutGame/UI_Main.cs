...
namespace ED
{
    public class UI_Main : SingletonDestroy<UI_Main>
    {
        ...
        private void ConnectBattle()
        {
            ...
            {
                NetworkManager.Get().ConnectServer(Global.PLAY_TYPE.BATTLE, NetworkManager.Get().LocalServerAddr, NetworkManager.Get().LocalServerPort, NetworkManager.Get().UserIdFromInspector);
                return;
            }
        ...
        private void ConnectCoop()
        {
            ...
            if (NetworkManager.Get().UseLocalServer == true)
            {
                NetworkManager.Get().ConnectServer(Global.PLAY_TYPE.COOP, NetworkManager.Get().LocalServerAddr, NetworkManager.Get().LocalServerPort, NetworkManager.Get().UserIdFromInspector);
                return;
            }

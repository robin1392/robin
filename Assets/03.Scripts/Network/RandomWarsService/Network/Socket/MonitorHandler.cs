...
namespace RandomWarsService.Network.Socket
{
    ...
    public class MonitorHandler
    {
        ...
        public void ShowResult()
        {
            ...
            _logger.Info("----------------------------------------------------------------------------------");
            _logger.Info("Network monitoring result");
            _logger.Info("----------------------------------------------------------------------------------");
            ...
            foreach (var user in _userPacketStatus)
            {
                ...
                {
                    _logger.Info(string.Format("Protocol: {0}, SendCount: {1:n0}, SendUsage: {2:n0} bytes, RecvCount: {3:n0}, RecvUsage: {4:n0} bytes", 
                        s.Value.ProtocolId, 
                        s.Value.SendCount, 
                        s.Value.SendUsage, 
        ...
        public void SetSendPacket(string sessionId, byte[] buffer)
        {
            lock (_lockObject)
            {
                ...
                _logger.Info(string.Format("[Monitor] SetSendPacket. sessionId: {0}, protocolId: {1}, length : {2}", sessionId, protocolId, length));
            }
        }
        ...
        {
            int totalLength = length + Defines.HEADER_SIZE;

            lock (_lockObject)
            {
                _totalRecvPacketUsage += totalLength;
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        _userPacketStatus[sessionId][protocolId].ReceiveUsage += totalLength;
                    }
                }

                _logger.Info(string.Format("[Monitor] SetReceivePacket. sessionId: {0}, protocolId: {1}, length : {2}", sessionId, protocolId, totalLength));
            }
        }
    }
}

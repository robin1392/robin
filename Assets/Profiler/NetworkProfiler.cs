...
namespace Mirror.Profiler
{
    ...
    public class NetworkProfiler
    {
        ...
        private GameObject GetGameObject(object message)
        {
            ...
            ClientObjectManager client = UnityEngine.Object.FindObjectOfType<ClientObjectManager>();
            ServerObjectManager server = UnityEngine.Object.FindObjectOfType<ServerObjectManager>();
            if (client != null && client.SpawnedObjects.TryGetValue(netId, out NetworkIdentity id))
            {
                return id.gameObject;
            }
            if (server != null && server.SpawnedObjects.TryGetValue(netId, out id))
            {
                return id.gameObject;
            }

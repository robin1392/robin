...
#region public static methods
		...
		private static UnityWebRequest GetWebRequest(Uri uri, RequestMethod method)
		{
			...
			if (method == RequestMethod.Head)
			{
				...
				{
					Debug.LogError(LogPrefix + "Couldn't get SDK version or set Accept-Encoding header.");
				}
			}
#endif
		...
		private static bool TryGetDate(string source, out DateTime date)
		{
			...
			{
				Debug.LogError(LogPrefix + "Error while parsing date: " + e);
		...
		private void StartDetectionInternal(float checkInterval, TimeCheatingDetectorEventHandler cheatCheckedEventHandler = null)
		{
			...
			{
				Debug.LogWarning(LogPrefix + "has properly configured Detection Event in the inspector, but still get started with TimeCheatingDetectorCallback callback. Both TimeCheatingDetectorCallback and Detection Event will be called on detection. Are you sure you wish to do this?", this);
			}

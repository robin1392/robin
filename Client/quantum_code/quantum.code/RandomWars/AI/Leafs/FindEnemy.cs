...
{
    [Serializable]
    public unsafe partial class FindEnemy : BTLeaf
    {
        ...
        protected override BTStatus OnUpdate(BTParams p)
        {
            ...
            Log.Info($"{currentTime} / {endTime}");
            if (currentTime < endTime)
            {
                Log.Info($"before move");
                var t = p.Frame.Unsafe.GetPointer<Transform2D>(p.Entity);
                t->Position += p.Frame.DeltaTime * FP._1 * Direction;
                Log.Info($"end move");

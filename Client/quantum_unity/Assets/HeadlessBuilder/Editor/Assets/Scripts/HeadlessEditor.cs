	...
	void OnGUI ()
	{
		...
		if (currentView == VIEW_SETTINGS) {
			...
			if (headlessSettings.applicationSection) {
                ...
                EditorGUILayout.BeginHorizontal();
                newSettings.valueLog = EditorGUILayout.ToggleLeft("", headlessSettings.valueLog, GUILayout.MaxWidth(13f));
                EditorGUILayout.LabelField("Strip debug logging from code", codeLabelStyle);
                EditorGUILayout.EndHorizontal();

                GUILayout.Space(3);
                GUILayout.Label("//Enhances performance by compiling out all debug logging.", commentStyle);
                EditorGUILayout.Space();
                EditorGUILayout.Space();
			...
			if (headlessSettings.assetsSection) {
                ...
                EditorGUILayout.BeginHorizontal();
                newSettings.valueShader = EditorGUILayout.ToggleLeft("", headlessSettings.valueShader, GUILayout.MaxWidth(13f));
                EditorGUILayout.LabelField("Strip shader variants (experimental)", codeLabelStyle);
                EditorGUILayout.EndHorizontal();

                GUILayout.Space(3);
                GUILayout.Label("//Greatly decreases build time (Unity 2018.2 and later).", commentStyle);
                EditorGUILayout.Space();
                EditorGUILayout.EndVertical();
				EditorGUILayout.EndVertical ();
			}
			...
			if (headlessSettings.assetsSection) {
                ...
                EditorGUILayout.BeginHorizontal();
                newSettings.valueDebug = EditorGUILayout.ToggleLeft("", headlessSettings.valueDebug, GUILayout.MaxWidth(13f));
                EditorGUILayout.LabelField("Attach to debuggers and profilers", codeLabelStyle);
                EditorGUILayout.EndHorizontal();

                GUILayout.Space (3);
				GUILayout.Label ("//Check this to output a development build.", commentStyle);
				EditorGUILayout.Space ();
				EditorGUILayout.EndVertical ();
				EditorGUILayout.EndVertical ();
			}
		...
		if (currentView == VIEW_ABOUT) {
			...
			if (GUILayout.Button ("Leave a review", linkCenterStyle)) {
				Application.OpenURL ("http://u3d.as/13uY");
			}
			EditorGUIUtility.AddCursorRect (GUILayoutUtility.GetLastRect (), MouseCursor.Link);

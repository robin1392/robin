	...
	public class ExamplesGUI : MonoBehaviour
	{
					...
					using (new GUILayout.VerticalScope())
					{
						...
						using (new GUILayout.HorizontalScope())
						{
							...
							using (new GUILayout.VerticalScope())
							{
								...
								using (new GUILayout.HorizontalScope())
								{
									obscuredPrefsExamples.UnobscuredMode = GUILayout.Toggle(obscuredPrefsExamples.UnobscuredMode, "unobscuredMode");
								}
#endif
								GUILayout.Space(5);
		...
		private void DrawDetectorsPage()
		{
			GUILayout.Label("ACTk is able to detect some types of cheating to let you take action on the cheating players. " +
			                "This example scene has all possible detectors and all of them are automatically start on scene start.");
			GUILayout.Space(5);
			using (new GUILayout.VerticalScope(GUI.skin.box))
			{

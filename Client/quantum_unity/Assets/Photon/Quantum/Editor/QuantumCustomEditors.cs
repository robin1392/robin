  ...
  public class AssetBaseEditor : UnityEditor.Editor {
    ...
    public override void OnInspectorGUI() {
      ...
      using (new QuantumEditorGUI.CustomEditorScope(serializedObject)) {
        ...
        QuantumEditorGUI.Inspector(serializedObject, filters: new[] { "Settings" }, drawScript: false);
        ...
        using (new QuantumEditorGUI.BoxScope(headline, serializedObject)) {
          ...
          QuantumEditorGUI.Inspector(serializedObject, "Settings");
        }
...
namespace Quantum.Editor {
  ...
  public class BinaryDataAssetEditor : AssetBaseEditor {
    ...
    public override void OnInspectorGUI() {
      ...
      QuantumEditorGUI.Inspector(serializedObject, new string[] { "Settings", "Source" });
      ...
      using (new QuantumEditorGUI.BoxScope(headline)) {
        ...
        var sourceProp = serializedObject.FindPropertyOrThrow(nameof(BinaryDataAsset.SourceTextAsset));
        var compressedProp = serializedObject.FindPropertyOrThrow("Settings." + nameof(BinaryData.IsCompressed));
        var dataProp = serializedObject.FindPropertyOrThrow("Settings." + nameof(BinaryData.Data));
      ...
      if (AssetDatabase.IsMainAsset(target.gameObject)) {
        if (NestedAssetBaseEditor.GetNested(target, out EntityPrototypeAsset asset)) {
          using (new QuantumEditorGUI.BoxScope(nameof(EntityPrototypeAsset))) {
            QuantumEditorGUI.Inspector(new SerializedObject(asset), "Settings", new[] { "Settings.Container" });
          }
        }
      }
          ...
          if (AssetDatabase.IsMainAsset(target.gameObject)) {
            if (NestedAssetBaseEditor.GetNested(target, out EntityViewAsset asset)) {
              using (new QuantumEditorGUI.BoxScope(nameof(EntityViewAsset))) {
                QuantumEditorGUI.Inspector(new SerializedObject(asset), "Settings");
              }
            }
          }
        }
  ...
  public class MapAssetEditor : AssetBaseEditor {
    public override void OnInspectorGUI() {
      if (!QuantumEditorSettings.Instance.UseQuantumAssetInspector) {
        ...
        QuantumEditorGUI.Inspector(serializedObject, new string[] { nameof(MapAsset.Settings), nameof(MapAsset.Prototypes) });
        using (new QuantumEditorGUI.BoxScope("Map")) {
          // This draws all fields except the "Settings" and script.
          QuantumEditorGUI.Inspector(serializedObject.FindPropertyOrThrow("Settings"), filters: new [] { "Settings.MapEntities" }, skipRoot: true);
          QuantumEditorGUI.Inspector(serializedObject.FindPropertyOrThrow(nameof(MapAsset.Prototypes)), skipRoot: false);
        }
      }
        ...
        if (data.CastRegion == MapNavMeshRegion.RegionCastType.CastRegion) {
          using (new GUILayout.HorizontalScope()) {
            using (new EditorGUI.DisabledScope(data.OverwriteCost == false)) {
              var property = serializedObject.FindProperty("Cost");
              property.Next(true);
              var rect = EditorGUILayout.GetControlRect();
              FPPropertyDrawer.DrawRaw(rect, property, new GUIContent("Cost"));
            }
    ...
    public override void OnInspectorGUI() {
      ...
      using (new QuantumEditorGUI.BoxScope("Photon Server Settings")) {
        ...
        using (new QuantumEditorGUI.SectionScope("App Settings")) {
          ...
          QuantumEditorGUI.Inspector(serializedObject, "AppSettings", new string[] { "AppSettings.AppIdChat", "AppSettings.AppIdVoice", "AppSettings.AppIdRealtime" }, skipRoot: false);
        }
    ...
    public override void OnInspectorGUI() {
      ...
      using (new QuantumEditorGUI.CustomEditorScope(serializedObject)) {
        using (new QuantumEditorGUI.BoxScope("Quantum Editor Settings")) {
          ...
          EditorGUILayout.LabelField(EditorGUIUtility.TrTextContentWithIcon("Experimental", MessageType.Warning), EditorStyles.boldLabel);
          {
...
namespace Quantum.Editor {
  ...
  public class SimulationConfigAssetEditor : UnityEditor.Editor {
    ...
    private class PathBuilder : SerializedPropertyPathBuilder<SimulationConfigAsset> { };
    private static readonly string SettingsPath           = PathBuilder.GetPropertyPath(asset => asset.Settings);
    private static readonly string PhysicsPath            = PathBuilder.GetPropertyPath(asset => asset.Settings.Physics);
    private static readonly string EntitiesPath           = PathBuilder.GetPropertyPath(asset => asset.Settings.Entities);
    private static readonly string NavigationPath         = PathBuilder.GetPropertyPath(asset => asset.Settings.Navigation);
    private static readonly string PhysicsLayersPath      = PathBuilder.GetPropertyPath(asset => asset.Settings.Physics.Layers);
    private static readonly string PhysicsLayerMatrixPath = PathBuilder.GetPropertyPath(asset => asset.Settings.Physics.LayerMatrix);
    ...
    public override void OnInspectorGUI() {
      ...
      using (new QuantumEditorGUI.CustomEditorScope(serializedObject)) {
        using (new QuantumEditorGUI.BoxScope("SimulationConfig")) {
          QuantumEditorGUI.Inspector(serializedObject, SettingsPath, filters: new string[] { PhysicsPath, EntitiesPath, NavigationPath });

          QuantumEditorGUI.Inspector(serializedObject, EntitiesPath, skipRoot: false);

          QuantumEditorGUI.Inspector(serializedObject, PhysicsPath, skipRoot: false, callback: (property, field, type) => {
            if (property.propertyPath == PhysicsLayerMatrixPath) {
              DrawLayersMatrix(property);
              return true;
            } else if (property.propertyPath == PhysicsLayersPath) {
          ...
          QuantumEditorGUI.Inspector(serializedObject, NavigationPath, skipRoot: false);
        }
      }
    }
  ...
  public class StaticEdgeCollider2DEditor : UnityEditor.Editor {
    ...
    private void DrawMovementHandles(QuantumStaticEdgeCollider2D collider) {
      ...
      Handles.matrix = Matrix4x4.TRS(
        t.position + collider.PositionOffset.ToUnityVector3(),
...
namespace Quantum.Editor {
  ...
  public class StaticPolygonCollider2DEditor : UnityEditor.Editor {
    ...
    private void DrawAddAndRemoveButtons(QuantumStaticPolygonCollider2D collider, bool drawAddButton, bool drawRemoveButton) {
      var handlesColor = Handles.color;
      var t = collider.transform;
      Handles.matrix = Matrix4x4.TRS(t.position + collider.PositionOffset.ToUnityVector3(), t.rotation, t.localScale);

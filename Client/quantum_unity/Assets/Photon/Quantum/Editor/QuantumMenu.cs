...
#pragma warning disable CS0618 // Type or member is obsolete
      if (mode.HasFlag(QuantumMapDataBakeFlags.Obsolete_BakeMapData)) {
#pragma warning restore CS0618 // Type or member is obsolete
        MapDataBaker.BakeMapData(data, true, 
          bakeColliders: true, 
          bakePrototypes: true);
      } else if (mode.HasFlag(QuantumMapDataBakeFlags.BakeMapPrototypes) || mode.HasFlag(QuantumMapDataBakeFlags.BakeMapColliders)) {
        MapDataBaker.BakeMapData(data, true, 
          bakeColliders: mode.HasFlag(QuantumMapDataBakeFlags.BakeMapColliders), 
          bakePrototypes: mode.HasFlag(QuantumMapDataBakeFlags.BakeMapPrototypes));
      }
      ...
      if (mode.HasFlag(QuantumMapDataBakeFlags.ImportUnityNavMesh) || mode.HasFlag(QuantumMapDataBakeFlags.BakeNavMesh)) {
      ...
      if (mode.HasFlag(QuantumMapDataBakeFlags.ClearUnityNavMesh)) {
        foreach (var navmeshDefinition in data.GetComponentsInChildren<MapNavMeshDefinition>()) {
      ...
      if (mode.HasFlag(QuantumMapDataBakeFlags.SaveUnityAssets)) {

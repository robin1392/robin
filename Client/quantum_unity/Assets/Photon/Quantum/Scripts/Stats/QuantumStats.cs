  ...
  void Update() {
    if (QuantumRunner.Default && Toggles[0].activeSelf) {
      ...
      if (QuantumRunner.Default.NetworkClient != null && QuantumRunner.Default.NetworkClient.IsConnected) {
        ...
        NetworkIn.text = (int)(QuantumRunner.Default.NetworkClient.LoadBalancingPeer.TrafficStatsIncoming.TotalPacketBytes / _networkTimer.Elapsed.TotalSeconds) + " bytes/second";
        NetworkOut.text = (int)(QuantumRunner.Default.NetworkClient.LoadBalancingPeer.TrafficStatsOutgoing.TotalPacketBytes / _networkTimer.Elapsed.TotalSeconds) + " bytes/second";
      }
    }
    else {
      _networkTimer = null;
